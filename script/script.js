 ...
 .then(data => {
  console.log(data);
   ...
   let minDifference = [];
     ...
     weatherRadius(data.list[minIndex], map, locID, service)

 })
 .catch(err => console.log('err', err))

}
function weatherRadius(data, map, locID, service){
  var requestArr = [];
  var selectedArr = [];
  console.log(data);

  const caferestaruant = ["cafe", "restaurant"];
  ...
  for(let i = 0; i < caferestaruant.length; i++){
  var request = {
    location: locID.location,
    radius: '500',
    keyword: caferestaruant[i]
  };
